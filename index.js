...
const filtrosAdicionados = [];

function adicionarDiv(event) {
    if (filtrosAdicionados.length >= 8) {
        alert('Número máximo de filtros atingido.');
        return;
    }

    const conteudo = event.target.parentNode.querySelector('p').textContent;

    if (filtrosAdicionados.includes(conteudo)) {
        alert('Esse filtro já foi adicionado. Selecione outro.');
        return;
    }
    ...
    filtrosAdicionados.push(conteudo);

    div.appendChild(removeImg);
    div.appendChild(paragraph);

    if (isLeftSpan) {
        document.getElementById('esquerda-span').appendChild(div);
    } else {
        document.getElementById('direita-span').appendChild(div);
    }

    isLeftSpan = !isLeftSpan;

    removeImg.addEventListener('click', () => {
        const index = filtrosAdicionados.indexOf(conteudo);
        if (index > -1) {
            filtrosAdicionados.splice(index, 1);
        }
...
addImages.forEach(image => {
    image.addEventListener('click', adicionarDiv);
});
